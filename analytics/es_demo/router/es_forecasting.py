...
# from es_demo.es_model import *
from utils.file_operations import *
import os
import json
from es_demo import analytics_es_config as config
...
def forecasting(test_size: int = Query(config.DEFAULT_VALUES['DEFAULT_TEST_SIZE'], description='Please provide the size of the test data'),
    ...
    try:
        ...
        for region in full_region_list:
            ...
            with open(f"{config.JSON_FILE_PATH['forecasting_file_path']}es_demo_model_results_{region}.json", 'w') as json_file:
            ...
            # dataframe_with_actual_predicted_and_validation.to_json(config.JSON_FILE_PATH['upload_file_path']+'es_demo_forecasting.json',orient='records')
            dataframe_with_actual_predicted_and_validation.to_json(f"{config.JSON_FILE_PATH['forecasting_file_path']}es_demo_forecasting_{region}.json", orient='records')
