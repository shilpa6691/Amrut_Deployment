...
def model_fitting(subsystem_name_for_pickle_file: str= Query(..., description="Please provide the subsystem name for the pickle file"),
    ...
    try:
        ...
        es_data_csv = file_decrypt(config.CSV_FILE_PATH['upload_file_path']+config.DATA_FILE_NAMES['processed_data_csv_file_name'],key)
        es_data = pd.read_csv(es_data_csv, index_col='DateTime', parse_dates=True)
