...
def es_storing_csv_pickle(#Upload_file: UploadFile = File('...'),
    ...
    try:
        zonewise_df_csv = file_decrypt(config.CSV_FILE_PATH['upload_file_path']+config.DATA_FILE_NAMES['processed_data_csv_file_name'],key)
        zonewise_df = pd.read_csv(zonewise_df_csv, index_col='DateTime', parse_dates=True)
